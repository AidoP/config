...
fn derive_struct(ident: &syn::Ident, s: &syn::DataStruct) -> TokenStream {
    ...
    let mut struct_ty = format!("{} {{ ", ident);
    let mut first = true;
    for field in s.fields.iter() {
        if let Some(ident) = &field.ident {
            ...
            let ty = ty.replace(" >", ">");
            let ty = ty.replace(" :: ", "::");
            struct_ty.push_str(&format!("{}: {}", ident, ty))
        } else {
        ...
        impl ::config::Struct for #ident {
            fn set<'a>(&mut self, field: &::config::Field<'a>) -> ::config::Result<'a, ()> {
                ...
                match field_name.as_ref() {
                    #(#field_setters,)*
                    _ => ::std::result::Result::Err(::config::Error::NoField(field.clone(), <Self as ::config::FromValue>::data_type()))
                }
            }
        }
                ...
                match value {
                    ...
                    ::config::Value::Struct { fields: field_list, ..} => {
                        ...
                        if let Some(&field) = fields.values().next() {
                            ::std::result::Result::Err(::config::Error::NoField(field.clone(), Self::data_type()))
                        } else {
...
fn derive_enum(ident: &syn::Ident, e: &syn::DataEnum) -> TokenStream {
    ...
    let constructors: Vec<_> = e.variants.iter()
        .map(|variant| {
            ...
            match &variant.fields {
                syn::Fields::Unit => 
                    quote!{ #variant_name => ::std::result::Result::Ok(Self::#variant_ident) },
        ...
        .collect();
    let setters: Vec<_> = e.variants.iter()
        .map(|variant| {
            let variant_ident = &variant.ident;
            // TODO: proper string case transformation
            let variant_name = variant.ident.to_string().to_lowercase();
            match &variant.fields {
                syn::Fields::Unit => 
                    quote!{ #variant_name => ::std::result::Result::Ok(*self = Self::#variant_ident) },
                syn::Fields::Unnamed(syn::FieldsUnnamed { unnamed, ..}) if unnamed.len() == 0 =>
                    quote!{ #variant_name => ::std::result::Result::Ok(*self = Self::#variant_ident()) },
                syn::Fields::Named(syn::FieldsNamed { named, ..}) if named.len() == 0 =>
                    quote!{ #variant_name => ::std::result::Result::Ok(*self = Self::#variant_ident{}) },
    ...
    quote!{
        impl ::config::FromValue for #ident {
            ...
            fn from_value_new<'a>(value: &::config::Value<'a>) -> ::config::Result<'a, Self> {
                let variant = value.clean(Self::data_type())?;
